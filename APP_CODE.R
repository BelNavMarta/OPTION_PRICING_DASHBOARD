...
#        Aproximación analítica: método M1/M2 de Hull.
...
# --- Asiática Aritmética: aproximación M1/M2 (Hull) --- 
  ...
  sidebarLayout(
    sidebarPanel(width = 3,
                 ...
                 conditionalPanel("input.producto=='vanilla' || input.producto=='barrier'",
                                  ...
                                  sliderInput("step_range", "Steps to display in tree", min=0, max=15, value=c(0,5), step=1)
                 ),
...
server <- function(input, output, session) {
  ...
  output$arbol_plot <- renderPlotly({
    ...
    layout(p, xaxis=list(title="Time step", dtick=1, zeroline = FALSE, showline = FALSE), yaxis=list(title = "", showticklabels = FALSE, showgrid = FALSE, zeroline = FALSE, showline = FALSE, ticks = "", visible = FALSE),
  ...
  output$arbol_bar_plot <- renderPlotly({
    ...
    layout(p,
           xaxis = list(title = "Time step", dtick = 1, zeroline = FALSE, showline = FALSE),
